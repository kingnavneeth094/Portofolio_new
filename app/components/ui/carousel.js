          ...
          <div className="fixed inset-0 h-screen z-50 overflow-auto">
            ...
            >
              ...
              <div className="py-10 flex flex-col">
                <p className="text-xl">{card.content?.desc}</p>

                <h3 className="mt-4 text-xl">Tech Stack Used:</h3>

                <ul className="list-disc pl-4">
                {card.techStack.map((tech,index)=>(
                  <li key={index} className="text-black">{tech}</li>
                ))}
                </ul>

                {/* Check if there are images to display */}
                {card.content?.img &&
                  Object.values(card.content.img).map((imageSrc, index) => (
                    <Image
                      key={index}
                      src={imageSrc}
